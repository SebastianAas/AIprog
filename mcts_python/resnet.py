...
class ResNet:
    ...
    def __init__(self, boardsize):
        self.model = self.get_resnet_hex(boardsize)
        self.model.compile(
            optimizer="ADAM",
            loss=MSE,
            metrics=['accuracy']
    ...
    def build(height,
        ...
        # create the model
        model = Model(input_data, pol_head, name="resnet")

        # return the constructed network architecture
        return model

    def get_resnet_hex(self, boardsize):
        """
        Method for return the model for hex
        :return: ResNet model for the game of hex
        """
        return self.build(height=boardsize,
                          width=boardsize,
                          depth=1,
                          filters=48,
                          policy_output_dim=boardsize**2)

    def train(self, x_train, y_train):
        self.model.fit(x_train, y_train, shuffle=True, epochs=5)
        return self.model
